...
outcome <- readRDS("./outcome_definition/Outputs_saved/aortic_dissection/output_final.rds") 
...
# Get protein columns
start_idx <- which(colnames(df_ad) == "AARSD1")
end_idx <- ncol(df_ad) - 1  # Second last column
protein_cols <- colnames(df_ad)[start_idx:end_idx]
...
na_counts <- colSums(is.na(df_ad[, model_cols]))
...
if(any(na_counts > 0)) {
    ...
    df_ad <- df_ad[complete.cases(df_ad[, model_cols]), ]
    cat("\nRows remaining after removing NAs:", nrow(df_ad), "\n")
}
...
# Create model matrix
X2 <- model.matrix(~ . - 1, data = df_ad[, c("sex.0.0", "age_at_recruitment.0.0", 
                                            "smoking_status_clean", "diabetes_clean", protein_cols)])
y <- df_ad$incident_case
...
X3 <- model.matrix(~ . - 1, data = df_ad[, c("sex.0.0", "age_at_recruitment.0.0", 
                                            "smoking_status_clean", "diabetes_clean",
...
roc1 <- roc(df_ad$incident_case, probs1)
auc1 <- auc(roc1)
...
# Create log file
log_file <- file.path(output_dir, "ad_results.log")
sink(log_file)

# Print all results to log file
cat("=== Analysis Results for AD ===\n\n")

cat("Sample size after removing NAs:", nrow(df_ad), "\n\n")

cat("Class distribution in outcome:\n")
print(table(df_ad$incident_case))
cat("\n")
...
results <- list(
    ...
    performance = list(
        ...
        auc_model2 = auc2,
        auc_model3 = auc3,
        class_distribution = table(df_ad$incident_case)
    )
)
saveRDS(results, file.path(output_dir, "ad_models.rds"))

# Create and save plots
pdf(file.path(output_dir, "ad_ridge_plots.pdf"))
par(mfrow = c(2,2))

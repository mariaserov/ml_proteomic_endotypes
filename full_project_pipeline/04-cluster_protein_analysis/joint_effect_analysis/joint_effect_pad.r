...
outcome <- readRDS("./outcome_definition/Outputs_saved/PAD/output_final.rds") 
...
# Get protein columns
start_idx <- which(colnames(df_pad) == "AARSD1")
end_idx <- ncol(df_pad) - 1  # Second last column
protein_cols <- colnames(df_pad)[start_idx:end_idx]
...
na_counts <- colSums(is.na(df_pad[, model_cols]))
...
if(any(na_counts > 0)) {
    ...
    df_pad <- df_pad[complete.cases(df_pad[, model_cols]), ]
    cat("\nRows remaining after removing NAs:", nrow(df_pad), "\n")
}
...
# Create model matrix
X2 <- model.matrix(~ . - 1, data = df_pad[, c("sex.0.0", "age_at_recruitment.0.0", 
                                            "smoking_status_clean", "diabetes_clean", protein_cols)])
y <- df_pad$incident_case
...
X3 <- model.matrix(~ . - 1, data = df_pad[, c("sex.0.0", "age_at_recruitment.0.0", 
                                            "smoking_status_clean", "diabetes_clean",
...
roc1 <- roc(df_pad$incident_case, probs1)
auc1 <- auc(roc1)
...
# Create log file
log_file <- file.path(output_dir, "pad_results.log")
sink(log_file)

# Print all results to log file
cat("=== Analysis Results for Peripheral Artery Disease ===\n\n")

cat("Sample size after removing NAs:", nrow(df_pad), "\n\n")

cat("Class distribution in outcome:\n")
print(table(df_pad$incident_case))
cat("\n")
...
results <- list(
    ...
    performance = list(
        ...
        auc_model2 = auc2,
        auc_model3 = auc3,
        class_distribution = table(df_pad$incident_case)
    )
)
saveRDS(results, file.path(output_dir, "pad_models.rds"))

# Create and save plots
pdf(file.path(output_dir, "pad_ridge_plots.pdf"))
par(mfrow = c(2,2))

# Question we are asking: Is the endotype's effect on the outcome more than the sum of its parts
rm(list = ls())
setwd("/rds/general/project/hda_24-25/live/TDS/Group09")
library(tidyverse)

# import data - ACTION REQUIRED - must select folder name
outcome <- readRDS("./outcome_definition/Outputs_saved/aortic_aneurysm/output_final.rds")
...
# Get protein columns
start_idx <- which(colnames(df_aa) == "AARSD1")
end_idx <- ncol(df_aa) - 1  # Second last column
protein_cols <- colnames(df_aa)[start_idx:end_idx]
...
na_counts <- colSums(is.na(df_aa[, model_cols]))
...
if(any(na_counts > 0)) {
    ...
    df_aa <- df_aa[complete.cases(df_aa[, model_cols]), ]
    cat("\nRows remaining after removing NAs:", nrow(df_aa), "\n")
}
...
# Create model matrix
X2 <- model.matrix(~ . - 1, data = df_aa[, c("sex.0.0", "age_at_recruitment.0.0", 
                                            "smoking_status_clean", "diabetes_clean", protein_cols)])
y <- df_aa$incident_case
...
X3 <- model.matrix(~ . - 1, data = df_aa[, c("sex.0.0", "age_at_recruitment.0.0", 
                                            "smoking_status_clean", "diabetes_clean",
...
roc1 <- roc(df_aa$incident_case, probs1)
auc1 <- auc(roc1)
...
# Create log file
log_file <- file.path(output_dir, "aa_results.log")
sink(log_file)

# Print all results to log file
cat("=== Analysis Results for Aortic Aneurysm ===\n\n")

cat("Sample size after removing NAs:", nrow(df_aa), "\n\n")

cat("Class distribution in outcome:\n")
print(table(df_aa$incident_case))
cat("\n")
...
results <- list(
    ...
    performance = list(
        ...
        auc_model2 = auc2,
        auc_model3 = auc3,
        class_distribution = table(df_aa$incident_case)
    )
)
saveRDS(results, file.path(output_dir, "aa_models.rds"))

# Create and save plots
pdf(file.path(output_dir, "aa_ridge_plots.pdf"))
par(mfrow = c(2,2))
